...
const schema: yup.ObjectSchema<LoginValues> = yup.object({
  email: yup.string().required('El email es un campo obligatorio').email(),
...
<template>
  <header class="mb-4 text-center">
    ...
    <p class="text-secondary mb-0">Accede a tu cuenta para gestionar tus compras</p>
  </header>
  ...
  <form class="card shadow-sm" @submit.prevent="onSubmit" novalidate>
    <div class="card-body p-4 p-md-5">
      ...
      <!-- Submit -->
      <div class="d-grid">
        <button type="submit" class="btn btn-primary btn-lg" :disabled="!canSubmit">Entrar</button>
      </div>
